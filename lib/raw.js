...
module.exports = function() {
  ...
  return through.obj(function each(file, enc, cb) {
    ...
    if (!metadata) {
      metadata = JSON.parse(fs.readFileSync(path.join(p[0], project, 'metadata.json')));
    ...
    if (type === 'references') {
      ...
      let promises = _.flatMap(dat.entries, ref => {
        return _.map(['input', 'output', 'schema'], type => {
          if (ref[type]) {
            ...
            let schema = JSON.parse(fs.readFileSync(path.join(p[0], project, '/schemas', sname)));
              ...
              resolve: {
                any: {
                  ...
                  read: (file, callback) => {
                    ...
                    callback(null, JSON.parse(fs.readFileSync(path.join(p[0], project, '/schemas', sname))));
                  },
                },
              },
            }).then(s => {
