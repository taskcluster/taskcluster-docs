...
module.exports = function(options) {
  ...
  return through.obj(function(file, enc, cb) {
    ...
    file.relative.split('/').forEach(function(name, idx) {
      path.push(name);
      // for reference docs, ignore 'references' and 'docs' at the 4th
      // position in the filename; these are just left out of the hierarchy
      // as in `mv reference/taskcluster-foo/docs/* reference/taskcluster/foo`
      if (options.rootPath === '/reference' && idx == 3 &&
    ...
    var addNav = function(node, parentNode) {
      ...
      if (node.file) {
        if (parentNode) {
          node.up = '/docs/' + parentNode.path;
        }
        if (prevNode) {
          node.prev = '/docs/' + prevNode.path;
          prevNode.next = '/docs/' + node.path;
        }
        prevNode = node;
        parentNode = node;
      }
    ...
    var makeToc = function(node) {
      return {
        ...
        inline: node.file? (node.file.inline || false) : false,
      }
    };
